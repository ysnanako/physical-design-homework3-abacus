...
prec NET::padHPWL()
{
    return (padpin_ur.x - padpin_lb.x) + (padpin_ur.y - padpin_lb.y);
}
...
void NET::updatePadPinPos()
{
    for(auto const &pin : pins)
    {
        if(!pin->pad)
            continue;
        padpin_lb.Min(pin->pad->center + pin->offset);
        padpin_ur.Max(pin->pad->center + pin->offset);
    }
}
...
unordered_set<TILE *> tile_set; // nanako
